...
bool maybe_raise_priority (struct thread *thread, int priority);
...
thread_lock_released (struct lock *lock)
{
  ...
  maybe_yield_to_ready_thread ();
=======
void thread_donate(struct thread *cur, int cur_priority)
{
  struct lock *acquired_lock = cur->lock_acquired_by_other_threads;
...
thread_set_priority (int new_priority) 
{
  ...
  if(new_priority > cur->priority)
  {
    /*Update new priority for current thread*/
    int max_priority_threads_needing_locks = thread_find_max_priority(cur);
    cur->priority = (new_priority > max_priority_threads_needing_locks) 
                    ? new_priority : max_priority_threads_needing_locks;
  }

  /*UPDATE PRIORITY WHEN THREAD PRIORITY IS NOT DONATED BY OTHER THREADS
  (LIST OF THREADS ACQUIRING LOCK HELD BY CURRENT THREAD IS EMPTY)*/

  if(new_priority < cur->priority)
  {
    int max_priority_threads_needing_locks = thread_find_max_priority(cur);
  
    /*thread_set_new_priority(cur, new_priority);*/
    cur->priority = (new_priority > max_priority_threads_needing_locks) 
                    ? new_priority : max_priority_threads_needing_locks;

    /*Yield current thread if priority < max thread priority in ready_list*/
    int max_priority_ready_list = find_max_priority_ready_list();
    if (!list_empty (&ready_list) && cur->priority < max_priority_ready_list)
      thread_yield ();

    /*Donate priority*/
    thread_donate(cur, cur->priority);
  }

  intr_set_level(old_level);
}

/*Find max thread priority in list threads waiting a lock held by the current thread*/
int
thread_find_max_priority (struct thread *cur)
{
  int max_priority = 0;
  struct list_elem *e;
  /*Loop through all threads acquiring the lock that held by current thread*/
  for (e = list_begin (&cur->lock_needed_by_other_threads); e != list_end (&cur->lock_needed_by_other_threads);
       e = list_next (e))
  {
    /*take the thread at the beginning of list*/
    struct lock *l = list_entry(e, struct lock, list_thread_waiting_lock);
    struct list_elem *take_thread_priority = list_front(&l->semaphore.waiters);
    struct thread *t  = list_entry(take_thread_priority, struct thread, elem);
...
int find_max_priority_ready_list()
{
  int max_priority = 0;
  struct list_elem *e;
   for (e = list_begin (&all_list); e != list_end (&all_list);
...
update_priority (struct thread *t)
{
  ...
  t->priority = FP_TO_INT(INT_TO_FP(PRI_MAX) - t->recent_cpu/4 - INT_TO_FP(t->nice * 2));
...
=======
  t->priority = priority;
  t->magic = THREAD_MAGIC;
//  list_init (&t->locks);  /*Care*/
  list_init (&t->lock_needed_by_other_threads);       /**/
>>>>>>> 930079f5cddde336bd0f6ae751434c089ff3a538
